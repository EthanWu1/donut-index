...
module.exports = {
  ...
  recordAuctionPriceSnapshot(listings, ts = Date.now()) {
    ...
    for (const listing of listings || []) {
      ...
      if (!key) continue;
      const stackPrice = lowestStackPriceForListing(listing);
      if (stackPrice <= 0) continue;
      const current = best.get(key);
      if (!current || stackPrice < current.lowestStackPrice) {
        best.set(key, {
          key,
          name: listing.name || key,
          lowestStackPrice: stackPrice,
        });
      }
    }
