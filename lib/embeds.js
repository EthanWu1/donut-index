...
// Custom-emoji string -> its CDN image URL (for use as an embed thumbnail).
// size=128 upscales the small native emoji so the thumbnail is not tiny.
...
const LB_EMOJI = {
  money: e.balance, shards: e.shards, kills: e.kills, deaths: e.deaths, playtime: e.playtime,
  placedblocks: e.placed, brokenblocks: e.broken, mobskilled: e.mobs,
  sell: e.iron_nugget, shop: e.gold_nugget,
};
...
// items: [{ name, key, amount, price, seller, timeLeft }]
function auctionEmbed(heading, items, footer) {
  // If every listing is the same item, show its icon as a thumbnail and drop
  // the repeated name from each line (the heading already names it).
  ...
  const lines = items.map((it) => {
    ...
    if (single) {
      return `\`$${formatNumber(it.price)}\`${amt} · sold by **${it.seller}**${tl}`;
    }
    const ic = itemEmoji(it.key);
    return `${ic ? `${ic} ` : ''}**${it.name}**${amt}\n`
      + `\`$${formatNumber(it.price)}\` · **${it.seller}**${tl}`;
  });
  const embed = new EmbedBuilder()
    .setColor(C.auction)
...
module.exports = {
  statsEmbed, leaderboardEmbed, auctionEmbed, historyEmbed, errorEmbed, emojiUrl, LB_EMOJI,
};
