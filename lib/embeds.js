...
// Invisible padding so narrow embeds stretch to the width of a dropdown.
...
function leaderboardEmbed(type, page, rows, callerIgn) {
  ...
  const head = `${LB_EMOJI[type] || ''} ${LB_TITLES[type] || type}`.trim();
  return new EmbedBuilder()
...
function auctionEmbed(heading, items, footer) {
  const lines = items.map((it) => {
    ...
    return `${ic ? `${ic} ` : ''}**${it.name}**${it.amount > 1 ? ` ×${it.amount}` : ''}\n`
      + `\`$${formatNumber(it.price)}\` · sold by **${it.seller}**`;
  });
  return new EmbedBuilder()
    .setColor(C.auction)
...
module.exports = { statsEmbed, leaderboardEmbed, auctionEmbed, historyEmbed, errorEmbed, WIDE };
