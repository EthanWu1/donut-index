...
function renderChart(points, opts = {}) {
  ...
  ctx.fillStyle = VALUE;
  ctx.font = `700 25px ${F}`;
  ctx.fillText(opts.title || 'History', padL, 35);
  ctx.fillStyle = AXIS;
  ctx.font = `500 13px ${F}`;
  ...
  ctx.lineTo(px(pts[pts.length - 1].ts), py(0));
  ctx.lineTo(px(pts[0].ts), py(0));
  ctx.closePath();
  const fill = ctx.createLinearGradient(0, padT, 0, H - padB);
  fill.addColorStop(0, 'rgba(255, 47, 120, 0.72)');
  fill.addColorStop(1, 'rgba(255, 47, 120, 0.12)');
  ctx.fillStyle = fill;
  ctx.fill();
  ...
  const drawn = [];
  const drawLabel = (marker) => {
    const p = pts.reduce((best, cur) =>
      (Math.abs(cur.ts - marker.ts) < Math.abs(best.ts - marker.ts) ? cur : best), pts[0]);
    const x = px(p.ts);
    const y = py(p.value);
    ctx.font = `700 11px ${F}`;
    const w = Math.ceil(ctx.measureText(marker.label).width) + 14;
    const h = 18;
    let lx = Math.max(padL + 2, Math.min(W - padR - w - 2, x - w / 2));
    let ly = y - 28;
    if (ly < padT + 2) ly = y + 13;
    for (const r of drawn) {
      if (Math.abs(lx - r.x) < w && Math.abs(ly - r.y) < h + 4) ly += h + 4;
    }
    ...
    ctx.fill();
    ctx.strokeStyle = marker.kind === 'latest' ? 'rgba(255,255,255,0.55)' : 'rgba(255,47,120,0.55)';
    ctx.lineWidth = 1;
    ctx.stroke();
    ctx.fillStyle = VALUE;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(marker.label, lx + w / 2, ly + h / 2 + 0.5);
  ...
  for (const marker of chartMarkers(pts, opts)) drawLabel(marker);

  return canvas.toBuffer('image/png');
}

module.exports = { renderChart, chartSummary, chartMarkers };
