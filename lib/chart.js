...
function chartMarkers(points, opts = {}) {
  ...
  const limit = Math.max(1, Number(opts.maxMarkers) || 10);
  const priority = {
    latest: 0, max: 1, min: 2, change: 3, turn: 4, first: 5,
  };
  const selected = out
    .slice()
    .sort((a, b) => (priority[a.kind] ?? 9) - (priority[b.kind] ?? 9) || b.ts - a.ts)
    .slice(0, limit);
  return selected.sort((a, b) => a.ts - b.ts || a.value - b.value);
}
