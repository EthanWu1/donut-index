...
const PREFIX_COMMANDS = new Set([
  'help', 'stats', 'profile', 'compare', 'history', 'rank', 'leaderboard',
...
function optionFacade(name, args, message) {
  ...
  return {
    getString(option) {
      ...
      if (['stats', 'profile', 'rank'].includes(name) && option === 'username') return args[0] || null;
      return null;
    },
    getInteger(option) {
...
function interactionFacade(message, name, args) {
  return {
    ...
    async reply(payload) {
      this.replied = true;
      return message.reply(cleanPayload(payload));
    },
